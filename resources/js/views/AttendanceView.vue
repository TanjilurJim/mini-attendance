...
const attendanceStats = computed(() => {
    ...
    const present = Object.values(attendance.value).filter(
        (s) => s === "present"
    ).length;
    const absent = Object.values(attendance.value).filter(
        (s) => s === "absent"
    ).length;
    const late = Object.values(attendance.value).filter(
        (s) => s === "late"
    ).length;
...
async function submitAttendance() {
    ...
    try {
        const records = Object.entries(attendance.value).map(
            ([studentId, status]) => ({
                student_id: parseInt(studentId),
                date: selectedDate.value,
                status: status,
                recorded_by:
                    JSON.parse(localStorage.getItem("me") || "{}").id || 1,
            })
        );
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <p class="mt-1 text-sm text-gray-600">
                    Mark student attendance for selected class
                </p>
            </div>
            ...
            <div v-else class="bg-white shadow rounded-lg p-12 text-center">
                ...
                <p class="text-gray-500">
                    Choose a class and section above to load students
                </p>
            </div>
        </div>
    </AppLayout>
